...
class Sim:
    ...
    **Main variables to coordinate with algorithm:**
    ...
    def generate_animation(self,pathFile):
        ...
        if self.map == None: self.load_map()
    ...
    def load_map(self, boundary={}):
        # Tau: 
        # boundary: a dictionary with keys "min_lat". "max_lat", "min_lng", "max_lng". If no such key, core.py detects by itself

        trk_bounds = self.user_tracks.get_bounds()

        if "min_lat" in boundary:
            min_lat = boundary["min_lat"]
        else:
            min_lat = trk_bounds.min_latitude
        
        if "max_lat" in boundary:
            max_lat = boundary["max_lat"]
        else:
            max_lat = trk_bounds.max_latitude

        if "min_lng" in boundary:
            min_lng = boundary["min_lng"]
        else:
            min_lng = trk_bounds.min_longitude

        if "max_lng" in boundary:
                ...
                for register in register_consumer_msg:
                    if msg.name == register["message_in"].name:
                        ...
                        if len(my_pipes[0].items) <= OFFLOAD_THRESHOLD:
                            ...
                            if(len(prev_pipes) == 0 or (len(next_pipes) > 0 and len(next_pipes[0].items) < len(prev_pipes[0].items))):
